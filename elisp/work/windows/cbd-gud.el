...
;; ab: my hack for remote  (append args (cons "-c" (cons "l+*;l-s" (cons "-lines" nil)))))
  (setq tmp (append (cl-loop for i in gud-cdb-options-hook append (funcall i)) (cons "-c" (cons "l+*;l-s" (cons "-lines" args)))))
;;  (debug)
 tmp)
  ...
  (cond
   ...
   (t ;; try to look the file up
    (let (;;(file-name-directory fname)
	  ...
	  (or
	   (cl-loop for i in gud-cdb-remap-fname-hooks
...
(defun cdb-promptPidAndExe (&optional exe-name-regexp predicate)
  ...
  (let
	...
	(setq exes
		  (cl-loop for n from 0 for i in (reverse (split-string tlist "\n"))
				collect
				(progn
				  (if (string-match "\\(^[ 0-9]+ \\)\\(.*\\)" i)
					  (cons (match-string 2 i) (match-string 1 i))))))
	(if exe-name-regexp
		(setq exe (cl-loop for i in exes if (string-match exe-name-regexp (car i)) return (car i)))
