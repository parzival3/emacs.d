;;; utils-banner.el --- ASCII Banner LETTERS -*- lexical-binding: t; -*-
...
(defun draw-ascii-banner-fn (packages?)
  (let* ((banner (make-banner))
    ...
    (let (
          (inhibit-read-only t)
          (loaded-packages (if packages? (get-loaded-packages-count) 0))
          (not-installed (if packages? (get-not-installed-packages-count) 0)))
      (erase-buffer)
      ;; Add top padding
...
(defun +banner--resize-handler (_)
  ...
  (when-let ((buffer (get-buffer "*Letters*"))
      ...
      (with-current-buffer buffer (draw-ascii-banner-fn nil))))

(provide 'utils-baner)
;;; utils-banner.el ends here
